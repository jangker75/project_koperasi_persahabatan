<x-admin-layout titlePage="{{ $titlePage }}">
                        ...
                        <div class="row mb-4">
                            ...
                            <div class="col-12 col-md-10">
                                <textarea name="note" class="form-control" id="note"
                                    placeholder="Tambahkan Catatan Opname" rows="4">{{ $opname->note }}</textarea>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <script>
        $(document).ready(function () {
            ...
            function renderFirstPage(order){
              $("#bodyTable").html("");
              order.forEach(element => {
                ...
                $("#bodyTable").append(renderElement(opname)) 
              });
            }

            function renderElement(product) {
              let checked = "";
              if(product.isExpired == 1){
                checked = `checked`
              }

              let type = "";
              if(product.type == 'plus'){
                type = `
              ...
              let elementHtml = `
                <tr data-id="` + product.id + `" data-code="` + product.code + `">
                  <td>` + product.name + `</td>
                  <td>
                    <input type="number" name="quantity" value="` + product.quantity + `" class="form-control qtyProduct" placeholder="Masukan Jumlah Temuan">
                  </td>
                  <td>
                    <select name="type" class="form-control form-select typeSelect" value="` + product.type + `" data-bs-placeholder="Pilih Tipe" id="type">
                        `+ type +`
                    </select>
                  </td>
                  <td>
                    <input type="checkbox" name="isExpired" class="checkboxExpired" `+ checked +` id="">
                    <label for="">Expired?</label>
                  </td>
                  <td>
                    <textarea type="text" name="description" class="form-control descriptionItem" rows="2" placeholder="Tambahkan Keterangan">` + product.description + `</textarea>
                  </td>
                  <td>
            ...
            $("body").on("click", ".btn-delete-row", function () {
                ...
                let code = String($(this).closest("tr").data("code")) 
              ...
              $.ajax({
                  ...
                  success: function (response) {
                    swal({
                      ...
                      setTimeout(function () {
                          window.location.replace("{{ url('admin/toko/opname') }}/{{ $opname->id }}");
                      }, 1000)
                  },
                  error: function (response) {
                    console.log(response)
